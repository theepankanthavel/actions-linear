 ...
 * @return Promise<void>
 */
export async function insertLabelToIssue(
  issueId: string,
  labels?: string[]
): Promise<void> {
  ...
  const [team, existingLabels] = await Promise.all([
    issue.team,
    (await issue.labels()).nodes,
  ]);
  if (!team?.id) {
    return;
  }
  ...
  const newLabelIds: string[] = await Promise.all(
    labels.map(async labelName => {
      try {
        const createIssueLabel = await linear.issueLabelCreate({
          teamId: team.id,
          name: labelName,
        });
...
async function getIssueLabelId(
  ...
  while (hasNextPage) {
    ...
    const issueLabel = response.nodes.find(
      node => node.name === labelName.trim()
    );
    if (issueLabel) {
    ...
    if (!hasNextPage) {
      throw new Error('Issue label id not found');
    }
